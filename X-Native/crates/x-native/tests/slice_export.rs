...
fn a_slice_exports_the_content_inside_its_bounds() {
    ...
    let plan = prepare_export(&page(), &vars, Some(&ids), &mut fonts).expect("slice exports");
...
fn an_empty_slice_still_exports_its_size() {
    ...
    let plan = prepare_export(&doc, &vars, Some(&ids), &mut fonts).expect("size only");
    assert_eq!((plan.width, plan.height), (30.0, 20.0));
    assert!(plan.tree.commands.is_empty(), "nothing overlaps it");
}
...
fn a_mixed_selection_refuses_to_guess() {
    ...
    let err = match prepare_export(&page(), &vars, Some(&ids), &mut fonts) {
        Err(e) => e,
