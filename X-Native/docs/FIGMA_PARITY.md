...
| behaviour | Figma source | our rule (owner) | pinned by |
| --- | --- | --- | --- |
| Select matching layers — the same layer across the page's frames — with ⌥⌘A | [Select matching objects](https://help.figma.com/hc/en-us/articles/360040449873-Select-layers-and-objects) | not built: no key handler, no `Editor` rule | open |
| A Section's title is a filled pill above the section's top-left, in the section's own colour — not a grey frame-style label | [Show names of nested frames](https://forum.figma.com/t/show-names-of-nested-frames/41949) | `scene.rs` Section arm draws the same label as a frame | open |
| Each frame carries its own "Show name" switch in the right sidebar's Layer section | [Hide frame names in Figma](https://designilo.com/figma/how-to-hide-frame-name-in-figma/) | not built: our rule is the two naming rules above, with no per-frame switch | open |
...
| An instance's internals carry no names | [Instance names are not displayed](https://forum.figma.com/t/why-isnt-the-name-of-an-instance-being-displayed/34665) | instance seeding sets `in_frame` | open |
| A name is canvas chrome and is never part of an export | [Section titles export?](https://forum.figma.com/t/section-titles-are-exporting-as-part-of-the-image/13338) | the export path drops the label nodes | open |
