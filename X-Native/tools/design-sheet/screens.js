...
function toolDock({ board = false, active = 'Select', top = false } = {}) {
  const tools = board ? BOARD_TOOLS : TOOLS;
  const w = board ? tools.length * 36 + 14 : 415;
  const source = board ? 'board_ui.rs' : 'editor_ui.rs';
  // Figma keeps the tools centred in the TOP bar; a floating bottom dock is our
  // divergence (18.8). The editor sheet draws Figma's target; boards keep theirs.
  const x = top ? (W - w) / 2 : CANVAS.x0 + (CANVAS.x1 - CANVAS.x0 - w) / 2;
  const y = top ? 0 : H - UI.toolbarBottom - UI.toolbarH;
  const h = top ? TITLE : UI.toolbarH;
  return at(x, y, w, h, `
    ${tools
      .map(
        ([name, ic, key]) =>
  ...
  `, { class: top ? 'dock top' : 'dock', attrs: `data-source="${source}"` });
}
...
function editorScreen(opts = {}) {
  ...
  return win(
    editorTitleBar(opts) +
      ...
      toolDock({ board: false, active: opts.tool || 'Select', top: true }) +
      (overlay || ''),
  );
}
